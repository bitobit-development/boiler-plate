...
function POSContent() {
  ...
  const { isAuthenticated, isLoading, user, session } = usePOSAuth();

  // Session heartbeat - sends heartbeat every 5 minutes to keep session alive
  usePOSSession(5 * 60 * 1000, isAuthenticated);
  ...
  // If not authenticated, hook will redirect to login
  // This is a safety check in case redirect hasn't happened yet
  if (!isAuthenticated) {
    return null;
  }
