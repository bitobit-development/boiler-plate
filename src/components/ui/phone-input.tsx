...
// Comprehensive list of 190+ countries with their dialing codes and flags
...
export function PhoneInput({
  ...
  React.useEffect(() => {
    if (value && value.startsWith("+")) {
      // Find matching country by dial code (sort by length descending to match longest first)
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // If user types a country code, auto-detect
    if (inputValue.startsWith("+")) {
      ...
      if (matchingCountry) {
        setSelectedCountry(matchingCountry);
        inputValue = inputValue.substring(matchingCountry.dialCode.length);
      }
    }
    ...
    let fullNumber = "";
    if (inputValue.startsWith("0") && selectedCountry.code === "ZA") {
      // SA local format: convert 0 to country code
      fullNumber = selectedCountry.dialCode + inputValue.substring(1);
    } else if (inputValue) {
      // International format
      fullNumber = selectedCountry.dialCode + inputValue.replace(/^0+/, "");
    }
  ...
  return (
    <div className={cn("flex gap-2", className)}>
      ...
      >
        <SelectTrigger
          className={cn(
            "w-[140px] bg-gray-800 text-white border-gray-700 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/50",
      ...
      <Input
        ...
        className={cn(
          "flex-1 bg-gray-800 text-white border-gray-700 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/50",
          error && "border-red-500"
        )}
        {...ariaProps}
      />
